...
use chrono::Utc;
use tipb::TopSQLSubResponse_oneof_resp_oneof;
...
pub struct TopSqlSubResponseParser2;

impl UpstreamEventParser for TopSqlSubResponseParser2 {
    type UpstreamEvent = tipb::TopSqlSubResponse;

    fn parse(response: Self::UpstreamEvent, instance: String) -> Vec<LogEvent> {
        match response.resp_oneof {
            Some(tipb::TopSQLSubResponse_oneof_resp_oneof::Record(record)) => Self::parse_tidb_record(record, instance),
            Some(tipb::TopSQLSubResponse_oneof_resp_oneof::SqlMeta(sql_meta)) => Self::parse_tidb_sql_meta(sql_meta),
            Some(tipb::TopSQLSubResponse_oneof_resp_oneof::PlanMeta(plan_meta)) => Self::parse_tidb_plan_meta(plan_meta),
            None => vec![],
        }
    }
}

impl TopSqlSubResponseParser2 {
