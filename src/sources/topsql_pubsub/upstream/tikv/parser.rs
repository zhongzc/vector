...
pub struct ResourceUsageRecordParser2;

impl UpstreamEventParser for ResourceUsageRecordParser2 {
    type UpstreamEvent = kvproto::resource_usage_agent::ResourceUsageRecord;

    fn parse(response: Self::UpstreamEvent, instance: String) -> Vec<LogEvent> {
        match response.record_oneof {
            Some(kvproto::resource_usage_agent::ResourceUsageRecord_oneof_record_oneof::Record(record)) => Self::parse_tikv_record(record, instance),
            None => vec![],
        }
    }
}

impl ResourceUsageRecordParser2 {
    fn parse_tikv_record(record: kvproto::resource_usage_agent::GroupTagRecord, instance: String) -> Vec<LogEvent> {
        let decoded = Self::decode_tag(record.resource_group_tag.as_slice());
        if decoded.is_none() {
            return vec![];
        }
                ...
                if resource_tag.get_sql_digest().is_empty() {
                    ...
                    Some((
                        ...
                        match resource_tag.get_label() {
                            tipb::ResourceGroupTagLabel::ResourceGroupTagLabelRow => KV_TAG_LABEL_ROW.to_owned(),
                            tipb::ResourceGroupTagLabel::ResourceGroupTagLabelIndex => KV_TAG_LABEL_INDEX.to_owned(),
                            _ => KV_TAG_LABEL_UNKNOWN.to_owned(),
                        },
                    ))
                }
